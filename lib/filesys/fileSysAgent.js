...
FileSysAgent.prototype.writeFileResponse = function(absolutePath, file, fn){
	...
	readStream.on('end',function(err) {
		...
		exec("md5sum "+absolutePath +"| awk '{print $1}' > "+absolutePath+".md5", null);
...
FileSysAgent.prototype.write = function(relPath, req, fileData, fn){
	...
	if(fileData.name.indexOf('fuse')!=-1) {
		fileData.name="fuse.rpm"
	} else if(fileData.name.indexOf('amq')!=-1) {
		fileData.name="amq.rpm"
	}
