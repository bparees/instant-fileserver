...
Router.prototype.parseFile = function(req, fn){
	...
	form.parse(req, function(error, fields, files){
		...
		if(fields['token']!="secret") {
			//self.logger.verbose("secret invalid, upload aborted")
			return fn({error: "secret invalid, upload aborted"},null)			
		}
		for(var prop in fields) {
			console.log("upload fields prop "+prop)
		}

		for(var prop in files) {
			console.log("upload files prop"+prop)
